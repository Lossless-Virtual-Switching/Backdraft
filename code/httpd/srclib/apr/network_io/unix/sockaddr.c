...
static apr_status_t get_local_addr(apr_socket_t *sock)
{
    ...
    ret = tas_getsockname(sock->socketdes, (struct sockaddr *)&sock->local_addr->sa,
                    &sock->local_addr->salen);
    if (ret == -1 && errno == EBADF)
        ret = getsockname(sock->socketdes, (struct sockaddr *)&sock->local_addr->sa,
                        &sock->local_addr->salen);
    if (ret < 0) {
...
static apr_status_t get_remote_addr(apr_socket_t *sock)
{
    ...
    ret = tas_getpeername(sock->socketdes, (struct sockaddr *)&sock->remote_addr->sa,
                    &sock->remote_addr->salen);
    if (ret == -1 && errno == EBADF)
        ret = getpeername(sock->socketdes, (struct sockaddr *)&sock->remote_addr->sa,
                        &sock->remote_addr->salen);
    if (ret < 0) {
        return apr_get_netos_error();
    }
