...
static apr_status_t socket_child_cleanup(void *sock)
{
    int ret;
    apr_socket_t *thesocket = sock;
    ret = tas_close(thesocket->socketdes);
    if (ret == -1 && errno == EBADF) {
        ret = close(thesocket->socketdes);
    }
    if (ret == 0) {
        thesocket->socketdes = -1;
        return APR_SUCCESS;
                               ...
                               int protocol, apr_pool_t *cont)
{
...
#ifndef BEOS_R5
    if (family != AF_INET  || ((type|flags) & ~(SOCK_NONBLOCK | SOCK_CLOEXEC)) != SOCK_STREAM) {
        (*new)->socketdes = socket(family, type|flags, protocol);
    }
    (*new)->socketdes = tas_socket(family, type|flags, protocol);
#else
...
#ifndef HAVE_SOCK_CLOEXEC
    {
        ...
        if ((flags = _fcntl((*new)->socketdes, F_GETFD)) == -1) {
            rv = errno;
            int ret;
            ret = tas_close((*new)->socketdes);
            if (ret == -1 && errno == EBADF)
                close((*new)->socketdes);
            (*new)->socketdes = -1;
            return rv;
        ...
        flags |= FD_CLOEXEC;
        if (_fcntl((*new)->socketdes, F_SETFD, flags) == -1) {
            rv = errno;
            int ret;
            ret = tas_close((*new)->socketdes);
            if (ret == -1 && errno == EBADF)
...
{
    int ret;
    ret = tas_shutdown(thesocket->socketdes, how);
    if (ret == -1 && errno == EBADF) {
        ret = shutdown(thesocket->socketdes, how);
    }
    return (ret == -1) ? errno : APR_SUCCESS;
}
...
{
    int ret;
    ret = tas_bind(sock->socketdes, (struct sockaddr *)&sa->sa, sa->salen);
    if (ret == -1 && errno == EBADF)
        ret = bind(sock->socketdes, (struct sockaddr *)&sa->sa, sa->salen);
...
{
    int ret;
    ret = listen(sock->socketdes, backlog); 
    if (ret == -1 && errno == EBADF) {
        ret = listen(sock->socketdes, backlog);
    }
                               ...
                               apr_pool_t *connection_context)
{
...
#ifdef HAVE_ACCEPT4
    {
        ...
        s = tas_accept4(sock->socketdes, (struct sockaddr *)&sa.sa, &sa.salen, flags);
        if (s == -1 && errno == EBADF)
            s = accept4(sock->socketdes, (struct sockaddr *)&sa.sa, &sa.salen, flags);
    }
#else
    s = tas_accept(sock->socketdes, (struct sockaddr *)&sa.sa, &sa.salen);
    if (s == -1 && errno == EBADF)
...
apr_status_t apr_socket_connect(apr_socket_t *sock, apr_sockaddr_t *sa)
{
    ...
    if ((rc == -1) && (errno == EINPROGRESS || errno == EALREADY)
...
#ifdef SO_ERROR
        {
            ...
            rc = tas_getsockopt(sock->socketdes, SOL_SOCKET, SO_ERROR, 
                                 (char *)&error, &len);
            if (rc == -1 && errno == EBADF)
                rc = getsockopt(sock->socketdes, SOL_SOCKET, SO_ERROR, 
                                 (char *)&error, &len);
            if (rc < 0) {
                return errno;
            }
