                             ...
                             apr_size_t *len)
{
    ...
    do {
        rv = tas_write(sock->socketdes, buf, (*len));
        if (rv == -1 && errno == EBADF) {
...
apr_status_t apr_socket_recv(apr_socket_t *sock, char *buf, apr_size_t *len)
{
    ...
    do {
        rv = tas_read(sock->socketdes, buf, (*len));
        if (rv == -1 && errno == EBADF) {
                               ...
                               apr_size_t *len)
{
    ...
    do {
        rv = tas_sendto(sock->socketdes, buf, (*len), flags, 
                                 ...
                                 apr_size_t *len)
{
    ...
    do {
        rv = tas_recvfrom(sock->socketdes, buf, (*len), flags, 
                              ...
                              apr_int32_t nvec, apr_size_t *len)
{
    ...
    do {
        rv = tas_writev(sock->socketdes, vec, nvec);
        if (rv == -1 && errno == EBADF)
...
#define sendfile sendfile64
    ...
    do {
        rv = tas_sendfile(sock->socketdes,    /* socket */
                                 ...
                                 apr_size_t *len, apr_int32_t flags)
{
    ...
    do {
        ...
        rv = tas_sendfile(file->filedes, /* file to be sent */
...
#define sendfile sendfile64
    ...
    do {
        if (nbytes) {       /* any bytes to send from the file? */
            ...
            rc = tas_sendfile(sock->socketdes,      /* socket  */
            ...
            rc = tas_writev(sock->socketdes, hdtrarray, 2);
            if (rv == -1 && errno == EBADF)
                                 ...
                                 apr_size_t *len, apr_int32_t flags)
{
    ...
    do {
        ...
        rv = tas_sendfilev(sock->socketdes, sfv, vecs, &nbytes);
        if (rv == -1 && errno == EBADF)
