...
def run_server(instance):
    ...
    cmd = 'sudo {bin} -l{cpu} --file-prefix={file-prefix} --vdev="{vdev}" --socket-mem=128 -- {sysmod} {mode} {delay}'.format(**args)
...
def run_client(ips):
    ...
    cmd = 'sudo {bin} -l{cpu} --file-prefix={file-prefix} --vdev="{vdev}" --socket-mem=128 -- {sysmod} {mode} {cnt_ips} {ips}'.format(**args)
