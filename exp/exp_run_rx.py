...
def moongen_run(config):
    ...
    cmd = 'sudo ./build/MoonGen examples/{script_to_load} --dpdk-config={dpdk_conf} {sender_dev} {receiver_dev} {sender} {receiver} --edrop {enable_drop} -r {main_workload_rate} -t {exp_duration}'.format(
            ...
            sender = config['is_sender'],
            receiver = config['is_recv'],
...
def sysbench(config):
    #cmd = "taskset --cpu-list 12,14,16 sysbench --test=cpu --cpu-max-prime=200000000 --num-threads=%s run &" %config["thread_number"]
...
def motive1(config_path):
    ...
    res_path = '/proj/uic-dcs-PG0/post-loom/exp/results'
    ...
    if(config.moongen['drop']):
        res_path = os.path.join(res_path, 'Backpressure')
    else:
        res_path = os.path.join(res_path, 'PFC')

    if(general_config.sysbench):
        res_path = os.path.join(res_path, 'non-isolated')
    else:
        res_path = os.path.join(res_path, 'isolated')

    if config.moongen['main_workload_rate'] < 5:
        res_path = os.path.join(res_path, str(config.moongen['main_workload_rate']*1000))
    else:
        res_path = os.path.join(res_path, str(config.moongen['main_workload_rate']))


    
    cmd = 'mkdir -p %s' %res_path
    subprocess.check_call(cmd, shell=True)
    cmd = 'yes | mv results/42_rx.csv results/FG_tx.csv results/pings.txt %s' %res_path
    subprocess.check_call(cmd, shell=True)
...
config = VhostConf(load_exp_conf('config/rx.json'))


#motive1("config/motive_config_1.json")
#motive1("config/motive_config_2.json")
#motive1("config/motive_config_3.json")
#motive1("config/motive_config_4.json")
#run_sysbench()
